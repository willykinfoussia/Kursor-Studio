...
**REQUIRED SUB-SKILL:** After this skill is loaded, `load_skill` `subagent-driven-brainstorming` and dispatch at least two `agent` calls (`subagent_type: "explore"`) in the same response before `ask_user_question` with `kind: "design"`. Research first; then present the design.
...
- After the human says yes/oui, do not restart brainstorming.
- You MUST invoke writing-plans with `load_skill`, then `subagent-driven-planning`, then call `create_plan` from the agreed design.
- writing-plans (and its research sub-skill) is the next step. Do not implement.
