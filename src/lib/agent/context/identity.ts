...
You MUST load_skill using-kursor-shell before run_command or start_process. Those tools run Windows cmd, not bash: one command per call, no pipes, no 2>&1, no output redirection. After start_process, call read_process with the jobId to see stdout and stderr. Do not look for log files. start_process / read_process / kill_process manage agent-owned jobs. Never use the human terminal.
